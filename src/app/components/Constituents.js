...
const Constituents = ({ handleOneConstituentClick }) => {
  ...
  // Filter and sort the list of constituents
  const filteredConstituents = constituents
    .filter((constituent) =>
      constituent.name.toLowerCase().includes(searchTerm.toLowerCase())
    )
    .sort((a, b) => {
      const aHasVideos = a.documents.some((doc) => doc.type === "video");
      const bHasVideos = b.documents.some((doc) => doc.type === "video");

      // Sort those with videos first
  ...
  return (
    ...
    >
      ...
      <div className="w-full flex justify-start items-start px-20 py-10">
        <div className="bg-white backdrop-blur-lg bg-opacity-60 shadow-lg rounded-2xl p-8 w-full h-full overflow-hidden border border-white border-opacity-30">
          ...
          <div className="max-h-[calc(90vh-120px)] overflow-y-auto">
            {filteredConstituents.map((constituent) => {
              const videoCount = constituent.documents.filter(
                (doc) => doc.type === "video"
              ).length;
              ...
              return (
                ...
                >
                  ...
                  <span className="bg-purple-100 text-purple-700 px-2 py-1 rounded-full text-sm font-medium mr-2">
                  ...
                  {videoCount > 0 && (
                    <span className="bg-blue-100 text-blue-700 px-2 py-1 rounded-full text-sm font-medium">
                      <Video className="inline mr-1 w-4 h-4" /> {videoCount}{" "}
                      Videos
                    </span>
                  )}
                </div>
